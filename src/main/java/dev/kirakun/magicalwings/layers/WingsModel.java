...
public class WingsModel<T extends LivingEntity> extends AgeableModel<T>
{
    ...
    public void setupAnim (T entity, float p_225597_2_, float p_225597_3_, float p_225597_4_, float p_225597_5_, float p_225597_6_)
    {
        ...
        else if ((entity instanceof AbstractClientPlayerEntity) && IsFlying((AbstractClientPlayerEntity)entity))
        {
            this.leftWing.z = -1f;
            f = 0.6981317F;
            f1 = (-(float) Math.PI / 4F);
    ...
    private boolean IsFlying(AbstractClientPlayerEntity player)
    {
        if (player.abilities.flying)
            return true;

        return MagicalWings.FlyingPlayers.getOrDefault(player.getId(), false);
    }
}
